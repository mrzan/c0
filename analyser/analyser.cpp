...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseMain() {
		...
		// <常量声明>
		//std::cout << "chang ling sheng ming" << std::endl;
		auto err = analyseConstantDeclaration();       
		if (err.has_value())
			return err;
		// <变量声明>
		//std::cout << "bian ling sheng ming" << std::endl;
		err = analyseVariableDeclaration();
		if (err.has_value())
			return err;
		// <语句序列>
		//std::cout << "yu ju xu lie" << std::endl;
	...
	std::optional<CompilationError> Analyser::analyseConstantDeclaration() {
		...
		while (true) {
			...
			// <常量声明语句>
			//std::cout << "chang liang sheng ming yu ju" << std::endl;
			next = nextToken();
			...
			// <常表达式>
			//std::cout << "chang biao da shi" << std::endl;
			int32_t val;
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		while (true) {
			...
			if (next.has_value() && next.value().GetType() != TokenType::EQUAL_SIGN) {
				unreadToken();
				addUninitializedVariable(tmp_next.value());
			}
				...
				if (err.has_value())
					return err;
				addVariable(tmp_next.value());
				_instructions.emplace_back(Operation::STO, getIndex(tmp_next.value().GetValueString()));
			}
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		int32_t tmp = std::any_cast<int>(std::stoi(next.value().GetValueString()));
		if (!isPositive)
			tmp = -tmp;
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		if (isUninitializedVariable(tmp_next.value().GetValueString())) {
			_vars[tmp_next.value().GetValueString()] = getIndex(tmp_next.value().GetValueString());
			_uninitialized_vars.erase(tmp_next.value().GetValueString());
		}
	...
	std::optional<CompilationError> Analyser::analyseFactor() {
		...
		if (next.value().GetType() == TokenType::PLUS_SIGN)
			...
			unreadToken();
		//std::cout << "fu hao = " << prefix << std::endl;
		...
		case TokenType::UNSIGNED_INTEGER:
			//std::cout << "wu fu hao zheng shu" << std::endl;
			_instructions.emplace_back(Operation::LIT, std::any_cast<int>(std::stoi(next.value().GetValueString())));
			break;
