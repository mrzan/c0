...
namespace miniplc0 {
	...
	std::pair<std::optional<Token>, std::optional<CompilationError>> Tokenizer::nextToken() {
		...
		while (true) {
			...
			//std::cout << "current char = " << current_char.value() << " state = " << current_state << std::endl;
			// 针对当前的状态进行不同的操作
			...
			case IDENTIFIER_STATE: {
				...
				// 如果读到的字符不是上述情况之一，则回退读到的字符，并解析已经读到的字符串
					...
					//     如果解析结果是关键字，那么返回对应关键字的token，否则返回标识符的token
					if (tmp == "begin") {
						//std::cout << "return begin " << tmp << std::endl;
						return std::make_pair(std::make_optional<Token>(TokenType::BEGIN, tmp, pos, currentPos()), std::optional<CompilationError>());
					}
					else if (tmp == "end")
						return std::make_pair(std::make_optional<Token>(TokenType::END, tmp, pos, currentPos()), std::optional<CompilationError>());
	...
	std::optional<char> Tokenizer::nextChar() {
		...
		_ptr = nextPos();
		//std::cout << "nextChar() = " << result << std::endl;
		return result;
	}
